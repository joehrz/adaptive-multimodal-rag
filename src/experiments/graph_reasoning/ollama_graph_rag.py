...
class OllamaGraphRAG:
    ...
    def __init__(
        ...
        try:
            ...
            if self.verbose:
                print(f"GraphRAG initialized with model: {self.model}")
...
Identify 2-5 key relationships:"""
        ...
        for line in response['response'].split('\n'):
            if 'RELATION:' in line and '->' in line:
                try:
                    ...
                    if len(rel_parts) >= 3:
                        source_name = rel_parts[0].strip().lower()
                        rel_type = rel_parts[1].strip().upper()
                        target_name = rel_parts[2].strip().lower()
...
List 1-5 relevant entity names, one per line:"""
        ...
        # Match entity names from response
        relevant_ids = []
    ...
    def query(self, query: str) -> GraphRAGResult:
        ...
        if self.verbose:
            ...
            print(f"Starting entities: {entity_names}")

        # Traverse graph
        if self.verbose:
            print(f"\nTraversing graph (max {self.max_hops} hops)...")
        visited_entities, reasoning_path = self._traverse_graph(start_entities, self.max_hops)

        if self.verbose:
            print(f"Visited {len(visited_entities)} entities")
            print(f"Found {len(reasoning_path)} relationships")
        ...
        # Generate answer
        prompt = f"""Answer this question using the knowledge graph information below.
...
Relationships (reasoning path):
{chr(10).join(relationship_context[:10])}

{f"Community Summaries:{chr(10)}{chr(10).join(community_context)}" if community_context else ""}

Based on this knowledge graph information, provide a comprehensive answer that follows the reasoning path:"""
        ...
        if self.verbose:
            ...
            print(f"Entities used: {len(result.entities_used)}")
